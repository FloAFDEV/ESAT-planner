-- ============================================================
-- MIGRATION CONSOLIDÉE — À appliquer en une seule fois
-- Ordre : colonnes → BL → workflow draft → split stock
-- ============================================================

-- ── ÉTAPE 1 : Colonnes manquantes ────────────────────────────────────────────

ALTER TABLE public.production_orders
  ADD COLUMN IF NOT EXISTS client_of_reference text;

ALTER TABLE public.shipments
  ADD COLUMN IF NOT EXISTS client_of_reference text;

-- ── ÉTAPE 2 : Numéro BL sur shipments ────────────────────────────────────────
...
SELECT
  s.id, s.reference, s.bl_number, s.status, s.client_id,
  c.name AS client_name, s.total_weight, s.total_pallets, s.created_at
FROM public.shipments s
LEFT JOIN public.clients c ON c.id = s.client_id;

CREATE OR REPLACE VIEW public.v_bl_summary AS
SELECT
  s.bl_number, s.client_id, c.name AS client_name,
  COUNT(s.id) AS shipment_count, SUM(s.total_weight) AS total_weight,
  SUM(s.total_pallets) AS total_pallets,
  MIN(s.created_at) AS first_shipment_at, MAX(s.created_at) AS last_shipment_at,
...
-- ── ÉTAPE 3 : can_start_now sur production_orders ────────────────────────────

ALTER TABLE public.production_orders
  ADD COLUMN IF NOT EXISTS can_start_now boolean DEFAULT NULL;

-- ── ÉTAPE 4 : Enum pending_material ──────────────────────────────────────────
